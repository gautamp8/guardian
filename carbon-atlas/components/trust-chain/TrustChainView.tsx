...
export function TrustChainView({ rootVcId }: TrustChainViewProps) {
  ...
  // Only show ghost "Token Minting" node if the policy has reached the monitoring stage
  const hasApprovedReport = chain.some((n) => n.entityType === "approved_report")
  const hasReport = chain.some((n) => n.entityType === "report" || n.entityType === "verification_report")
  const showGhost = hasReport || hasApprovedReport

  const totalSteps = chain.length + (showGhost ? 1 : 0)
  ...
  return (
    <div className="space-y-0">
      ...
      {showGhost && <GhostStep stepNumber={totalSteps} />}

      {chain.map((node, i) => {
        const stepNumber = (showGhost ? totalSteps - 1 : totalSteps) - i
